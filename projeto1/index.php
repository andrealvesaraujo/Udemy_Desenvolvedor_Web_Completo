...
$app->get('/produtos',function(){
    ...
    foreach ($data as &$produto) {
        $preco = $produto['preco'];//$produto['preco'] ja existe essa coluna no banco
        $centavos = explode(".", $produto['preco']);//E utilizando ela faz essas novas variaveis
        $produto['preco'] = number_format($produto['preco'], 0, "," ,".");//Trocando o ponto por virgula
        $produto['centavos']= end($centavos);//todas essas variaveis são novas e utilizadas no shop
        $produto['parcelas'] = 10;
...
$app->get("/produto-:id_prod",function($id_prod){
    ...
    $preco = $produto['preco'];
    $centavos = explode(".", $produto['preco']);
    $produto['preco'] = number_format($produto['preco'], 0, "," ,".");//Em ordem:nome da variavel que vai mudar,numero de casas decimais,sinal de decimal,sinal de milhar
    $produto['centavos']= end($centavos);
    $produto['parcelas'] = 10;
    $produto['parcela'] = number_format($preco/$produto['parcelas'], 2 , ",", ".");
    $produto['total'] = number_format($preco, 2, "," ,".");
...
$app->get("/calcular-frete-:cep", function($cep){
    ...
    require_once("inc/php-calcular-frete-correio/Frete.php");
